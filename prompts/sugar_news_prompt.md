...
Brazil sugar hedging progress July 19 2026
Brazil mills hedged sugar July 19 2026
...
Brasil hedge açúcar usinas 19 de julho de 2026
Brasil fixação contratos exportação açúcar 19 de julho de 2026
...
中国必须作为独立重点国家检索和检查，不得归入其他国家。中国检索不得只使用英文关键词，中文关键词至少覆盖：
...
中国新闻数量不受限制。若目标日期没有中国重要新增信息，直接省略中国数据行，不写占位文字，但后台日志必须记录中国检索已完成。结构化数据中中国必须使用独立对象和 `country_group=中国`，不得保存到“其他国家”。
...
新闻页面开头必须输出 `全球糖业新闻重点`，用 2-3 句中文概括当日最重要的国家、事件和核心数据，说明主要利多、利空因素，最后概括国际糖价的主要矛盾或运行判断。不得逐条复述后文，不得使用空泛市场套话；只有 ICE 原糖持续站上 `15美分/磅` 时才使用“震荡偏强”。

从 2026-08-16、2026-08-17、2026-08-18 和 2026-08-31 的重写版本开始，后续日报固定按这个模板总结：

1. 全局重点第一句写当日最重要国家和事件类型；第二句把最关键数据放进利多/利空对比；第三句写国际糖价主要矛盾，未确认 ICE 原糖持续站上 `15美分/磅` 时只写“震荡”。当印度国内糖价持续创下新高但 ICE 原糖尚未确认持续站上 `15美分/磅` 时，结尾用“ICE原糖持续站上15美分/磅前以震荡判断为宜”，不要写“ICE原糖未持续站上15美分/磅前以震荡判断为宜”。
2. 单条新闻第一句必须写主体、动作、国家/产区/周期和核心数据；第二句必须写同一事件如何通过产量、库存、进口、出口、乙醇分流、消费或天气影响供需和糖价；第三句只用于说明项目未落地、政策尚未执行、天气阶段差异或短期/中长期差异。
3. 国家归属按事件发生地和主要市场影响判断，不按媒体或次要合作方判断。例如尼日利亚 NSDC 投资计划归入“尼日利亚/其他国家”，即使正文提到外部 EPC 或融资合作方。
4. 价格新闻不要误写成库存新闻。若原文是原糖 `16.6美分/磅`、白糖创阶段高位、孟买现货 `5000-5090卢比/公担`、一个月上涨近 `10%`，应写价格上涨反映阶段性供需偏紧和需求补库，不能写“库存增加”。
5. 乙醇政策要区分方向：提高 E20 以上掺混或扩大糖基乙醇需求会分流甘蔗汁、糖浆或 B 重糖蜜，利多糖价；谨慎推进 E20 以上或支持 E10 会减少糖基乙醇增量需求，使更多可结晶蔗糖用于制糖，偏利空。
6. 投资、复产、糖厂重建或本土产量目标要写明时间维度：中长期增加压榨或产糖能力偏利空；若仍在审批、融资、重建或准备阶段，要说明短期供应影响有限。

按 2026-08-18 自动生成版与重写版的差异，后续必须执行以下纠偏规则：

1. 不得写“相关机构”“糖业市场公布”“相关数值为”。原文有主体和指标时，必须写真实主体和指标，例如印度政府、印度农业与农民福利部、旁遮普邦糖业市场、科尔哈普尔糖厂、尼泊尔糖业市场。
2. 同一事件多来源转载时先去重。例如 Moneycontrol 与 APAC Media 同时报道印度考虑削减/取消 `100%` 食糖进口税，只保留一条，不得在全球重点和印度栏目重复成两条贸易政策新闻。
3. 进口税、配额、关税新闻必须写政策状态和传导路径。印度在国内糖价创纪录时评估削减或取消 `100%` 食糖进口税，首先判断为利多糖价：这证明印度国内糖源偏紧、价格高企，短期强化供应紧张预期；同时说明若政策真正落地并形成进口流入，后续会补充国内可用糖源、压制印度现货糖价。
4. 提前开榨不能机械判断为增加供应。若原文写提前 `15天` 开榨可能使回收率下降 `1-1.5个百分点`、减少产糖并造成 `700-800卢比/吨` 损失，应按“降低同等甘蔗压榨量下食糖产出、利多糖价”处理。
5. 创纪录价格、前期价格对比、缺货、库存偏薄、节庆需求上升通常不是中性。旁遮普 `60卢比/公斤` 批发、`65卢比/公斤` 零售且一周前 `54-56卢比/公斤`、一个月前 `46-48卢比/公斤`，应写供应紧张和囤货压力支撑糖价。科尔哈普尔 `4200-4500卢比/公担` 糖厂价和 `52-55卢比/公斤` 零售价，应写库存偏薄和节庆需求支撑现货。
6. 甘蔗面积新闻必须同时比较去年同期和正常面积。甘蔗种植面积增加意味着下一季糖料和食糖产量预期提高，原则上利空糖价。印度甘蔗面积 `5831万公顷` 低于去年 `5862万公顷`、但高于正常 `5420万公顷`，且北方邦、哈里亚纳面积增加时，应写面积高于正常和主产区扩种带来的供应压力，判断为利空；马哈拉施特拉、旁遮普减少只用于说明利空幅度受限，不能写“方向不明”。
7. 短缺新闻不能因缺少官方库存表就写中性。尼泊尔糖价由 `95-100卢比/公斤` 升至 `120卢比/公斤`，节日消费预计翻倍且进口安排延迟，应写当地可用糖源收紧、进口需求增加，利多糖价。
8. 次要国家不能覆盖事件国家。尼泊尔新闻即使提到依赖进口，也归入“尼泊尔/其他国家”；正文以尼泊尔本地库存、进口审批、价格和节日需求为主，避免让次要来源国触发错误归类。
9. 全球重点必须基于去重和纠偏后的事实重写，不能照搬自动生成的“某国相关机构+相关数值”类别串联。
10. 印度天气和病虫害新闻不能只看“强降雨”四个字。北方邦、马哈拉施特拉、卡纳塔克等主产区若出现强降雨引发红腐病、病害、虫害、积水、倒伏、洪涝、收割或运输受阻，应按甘蔗单产或糖料供应受损处理，判断为利多糖价；只有普通生长期降雨、且没有病害/受损/洪涝/收割受阻措辞时，才按改善墒情、利空糖价处理。
11. 自动摘要不得再输出“相关数值为”“糖价相关数值为”“进口、出口、关税或配额变化会改变国内外可用糖源和贸易流向”“价格上行通常会抬高补库成本”等模板句。必须写清具体主体、政策、数值和传导逻辑。
12. 2026-08-31 重写版是明日及后续自动日报的最新样板：全球重点最多 3 句，第一句写主导国家和事件类型，第二句把核心利多/利空数字放在同一判断框架内，第三句写 ICE 原糖主要矛盾；印度 `9月上半月130万吨` 销售配额、首周 `40%` 销售要求和同日糖厂报价属于同一国内供应调控逻辑时应合并；巴西 `CEPEA` 乙醇价格、巴西生物燃料价格恢复或 ICE 原糖收盘必须写清甘蔗分配、乙醇分流、出口平价或短期供应路径；中国报价和灾情新闻必须写具体地区、产品、价格/涨跌、受灾主体、面积或糖料供应路径。不得输出 `糖业市场发布或调整食糖价格和市场流通`、`糖价或相关政策基准为XXX`、`糖业相关机构公布库存数据`、`库存数据，库存增加` 或 `食糖价格上涨说明现货供应偏紧或节前需求增强，价格下跌则说明供应压力或需求转弱正在传导到现货端`。
...
B 列新闻每条 2-3 句，并在末尾保留来源和影响判断：

```text
来源：机构或媒体名称（原始链接）。影响：利多糖价 / 利空糖价 / 中性
```
...
5. 不存在重复、旧闻或无关内容；
6. 数据、单位和来源准确；
7. 每条新闻正文末尾都有 `影响：利多糖价 / 利空糖价 / 中性`；
8. 新闻与影响简洁；
9. 新增行继承模板格式；
10. 文字完整显示；
11. 已核验新闻全部写入 Excel；
12. 保存后重新读取 Excel 的数量与核验清单一致；
13. 如有中国新闻，是否在 Excel、结构化数据、本地看板和 Vercel 线上看板中一致；如无中国新闻，后台日志是否记录中国检索完成；
14. 其他国家新闻是否全部用具体国家或地区名称展示；
15. Excel、结构化数据、本地看板和 Vercel 线上看板是否逐条一致；
16. 是否不存在血糖、糖尿病、胰岛素、医学健康或营养保健新闻；
17. 是否不存在按来源网站或转载媒体所在地误分国家的新闻。
