...
For import premium, discover articles from the HiSugar list/interface, open the matching article titled like `YYYYMMDD食糖进口成本及利润估算`, and extract the `进口升贴水` row from the report image/table. The dashboard card date must be the internal row/report date such as `20260724`; article publish time, crawl time, page order, Vercel deployment time, and example dates must never replace it. The normal 06:00 Beijing run uses reports available at that time. Because HiSugar may publish a report later in the morning, run `scripts/brazil_sugar_metrics.py --date YYYY-MM-DD --premium-only` at 10:00 Beijing time and accept a newly published article when its internal data date is on or before the Sugar News report date. That late refresh may update only the Brazil premium snapshot and Brazil dashboard block; it must not refetch stocks, regenerate other countries' news, or touch other dashboards.
